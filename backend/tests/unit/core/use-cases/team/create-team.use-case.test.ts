...
describe('CreateTeamUseCase', () => {
  ...
  it('should create a team successfully', async () => {
    ...
    expect(result.getValue().name).toBe(createTeamDto.name);
    expect(result.getValue().tournamentId).toBe(createTeamDto.tournamentId);
    expect(result.getValue().players).toEqual(createTeamDto.players);
  ...
  it('should fail if team name already exists in tournament', async () => {
    ...
    expect(result.isSuccess).toBe(false);
    expect(result.isFailure).toBe(true);
    expect(result.getError().message).toBe('Ya existe un equipo con ese nombre en el torneo');
    expect(teamRepository.teams.length).toBe(1);
  });
  ...
  it('should fail if team validation fails', async () => {
    ...
    expect(result.isSuccess).toBe(false);
    expect(result.isFailure).toBe(true);
    expect(result.getError().message).toBe('El nombre del equipo es obligatorio');
    expect(teamRepository.teams.length).toBe(0);
  });
}); 