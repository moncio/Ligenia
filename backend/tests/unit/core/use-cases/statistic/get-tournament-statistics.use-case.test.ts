import { GetTournamentStatisticsUseCase } from '../../../../../src/core/use-cases/statistic/get-tournament-statistics.use-case';
...
describe('GetTournamentStatisticsUseCase', () => {
  ...
  it('should get all statistics for a tournament', async () => {
    const result = await useCase.execute('tournament-1');

    expect(result.isSuccess).toBe(true);
    expect(result.getValue()).toBeDefined();
    expect(result.getValue().length).toBe(2);
    expect(result.getValue()[0].id).toBe('statistic-1');
    expect(result.getValue()[1].id).toBe('statistic-2');
    expect(result.getValue()[0].tournamentId).toBe('tournament-1');
    expect(result.getValue()[1].tournamentId).toBe('tournament-1');
  });
  ...
  it('should return empty array if no statistics exist for the tournament', async () => {
    // Añadir un torneo sin estadísticas
    tournamentRepository.tournaments.push(
      new Tournament({
        id: 'tournament-3',
        name: 'Torneo 3',
