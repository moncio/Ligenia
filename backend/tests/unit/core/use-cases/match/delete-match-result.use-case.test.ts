import { DeleteMatchResultUseCase } from '../../../../../src/core/use-cases/match/delete-match-result.use-case';
...
describe('DeleteMatchResultUseCase', () => {
  let useCase: DeleteMatchResultUseCase;
  let matchRepository: MockMatchRepository;

  beforeEach(() => {
    matchRepository = new MockMatchRepository();
    useCase = new DeleteMatchResultUseCase(matchRepository);
  ...
  it('should delete a match result successfully', async () => {
    const result = await useCase.execute('match-2');

    expect(result.isSuccess).toBe(true);
    expect(result.getValue()).toBeDefined();
    expect(result.getValue().score).toBeUndefined();
  ...
  it('should fail if match does not have a result', async () => {
    const result = await useCase.execute('match-1');

    expect(result.isSuccess).toBe(false);
    expect(result.isFailure).toBe(true);
    expect(result.getError().message).toBe('El partido no tiene un resultado para eliminar');
  });
}); 