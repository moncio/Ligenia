import { UpdateTournamentMatchesAndStandingsUseCase, UpdateTournamentMatchesAndStandingsInput } from '../../../../src/core/application/use-cases/tournament/update-tournament-matches-and-standings.use-case';
...
describe('UpdateTournamentMatchesAndStandingsUseCase', () => {
  ...
  function createMatch(id: string, tournamentId: string, round: number, 
                       homePlayerId: string, awayPlayerId: string, 
                       homeScore: number | null = null, awayScore: number | null = null, 
                       status: MatchStatus = MatchStatus.PENDING): Match {
  ...
  describe('Success cases', () => {
    it('should update tournament and create next round matches when round 1 is complete', async () => {
      ...
      const match1 = createMatch(uuidv4(), tournamentId, 1, player1, player2, 2, 0, MatchStatus.COMPLETED);
      const match2 = createMatch(uuidv4(), tournamentId, 1, player3, player4, 2, 1, MatchStatus.COMPLETED);
    ...
    it('should handle odd number of winners with bye match', async () => {
      ...
      const match1 = createMatch(uuidv4(), tournamentId, 1, player1, player2, 2, 0, MatchStatus.COMPLETED);
      const match2 = createMatch(uuidv4(), tournamentId, 1, player3, player4, 2, 1, MatchStatus.COMPLETED);
      const match3 = createMatch(uuidv4(), tournamentId, 1, player5, '', 2, 0, MatchStatus.COMPLETED); // Bye match
    ...
    it('should complete the tournament when the final match is done', async () => {
      ...
      const finalMatch = createMatch(uuidv4(), tournamentId, 2, player1, player3, 2, 0, MatchStatus.COMPLETED);
  ...
  describe('Failure cases', () => {
    ...
    it('should fail if not all matches in current round are completed', async () => {
      ...
      const match1 = createMatch(uuidv4(), tournamentId, 1, player1, player2, 2, 0, MatchStatus.COMPLETED);
      const match2 = createMatch(uuidv4(), tournamentId, 1, player3, player4, null, null, MatchStatus.PENDING);
