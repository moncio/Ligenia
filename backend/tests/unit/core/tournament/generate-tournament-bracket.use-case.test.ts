...
describe('GenerateTournamentBracketUseCase', () => {
  ...
  function createAdminUser(userId: string): User {
    const user = new User(userId, 'admin@example.com', 'Admin', 'User', UserRole.ADMIN, true);
    return user;
  }

  // Helper function to create a regular user
  function createRegularUser(userId: string): User {
    const user = new User(userId, 'user@example.com', 'Regular', 'User', UserRole.PLAYER, true);
    return user;
  }
