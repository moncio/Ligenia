import { UpdateRankingsAfterMatchUseCase, UpdateRankingsAfterMatchInput } from '../../../../src/core/application/use-cases/ranking/update-rankings-after-match.use-case';
...
describe('UpdateRankingsAfterMatchUseCase', () => {
  ...
  beforeEach(() => {
    ...
    calculatePlayerRankingsUseCase = new MockCalculatePlayerRankingsUseCase([
      homePlayer1Id, homePlayer2Id, awayPlayer1Id, awayPlayer2Id
    ]);
  ...
  it('should fail when trying to update rankings for a non-completed match', async () => {
    ...
    expect(result.getError().message).toBe('Cannot update rankings for a match that is not completed');
  });
  ...
  it('should handle singles matches correctly', async () => {
    ...
    const input: UpdateRankingsAfterMatchInput = { matchId: '123e4567-e89b-12d3-a456-426614174012' };
