...
export type UpdateTournamentMatchesAndStandingsInput = z.infer<typeof updateTournamentMatchesAndStandingsSchema>;
  ...
  protected async executeImpl(
    ...
    try {
      ...
      if (allMatches.length === 0) {
        return Result.fail(
          new Error(`No matches found for tournament ${tournamentId}`)
        );
      }
      ...
      if (!allMatchesCompleted) {
        return Result.fail(
          new Error(`Not all matches in round ${currentRound} are completed yet`)
        );
      }
