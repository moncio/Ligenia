  ...
  protected async executeImpl(
    ...
    try {
      ...
      if (tournament.status === TournamentStatus.ACTIVE) {
        return Result.fail(
          new Error('Cannot cancel an active tournament')
        );
      }

      if (tournament.status === TournamentStatus.COMPLETED) {
        return Result.fail(
          new Error('Cannot cancel a completed tournament')
        );
      }

      if (tournament.status === TournamentStatus.CANCELLED) {
        return Result.fail(
          new Error('Tournament is already cancelled')
        );
      }
      ...
      return Result.fail(
        error instanceof Error 
          ? error 
          : new Error('Failed to cancel tournament')
      );
    }
  }
} 