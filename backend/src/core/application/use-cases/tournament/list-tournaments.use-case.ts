...
> {
  ...
  protected async executeImpl(input: ListTournamentsInput): Promise<Result<ListTournamentsOutput>> {
    try {
      // Validate input
      ...
      return Result.fail(error instanceof Error ? error : new Error('Failed to list tournaments'));
    }
  }
}
