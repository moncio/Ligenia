...
import { z } from 'zod';
import { Tournament, TournamentFormat, TournamentStatus, PlayerLevel } from '../../../domain/tournament/tournament.entity';
...
export class UpdateTournamentUseCase extends BaseUseCase<UpdateTournamentInput, Tournament> {
  ...
  protected async executeImpl(input: UpdateTournamentInput): Promise<Result<Tournament>> {
    try {
      ...
      if (!tournament) {
        return Result.fail<Tournament>(new Error(`Tournament with ID ${validatedData.id} not found`));
      }
      ...
      if (registrationDeadline !== undefined && registrationDeadline !== null && 
        ...
        return Result.fail<Tournament>(new Error('Registration deadline must be before start date'));
      }
