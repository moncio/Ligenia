  ...
  protected async executeImpl(
    ...
    try {
      ...
      if (validatedData.playerId) {
        ...
        if (!player) {
          return Result.fail<CalculatePlayerRankingsOutput>(new Error('Player not found'));
        }
        ...
        const rankedStatistics = allStatistics
          .map(s => {
            const points =
              s.winRate * 0.4 + s.averageScore * 0.3 + s.tournamentsWon * 15 + s.matchesWon * 0.3;
            return {
              playerId: s.playerId,
              points,
            };
        ...
        const globalPosition =
          rankedStatistics.findIndex(s => s.playerId === validatedData.playerId) + 1;
        ...
        const categoryPosition =
          categoryRankedStatistics.findIndex(s => s.playerId === validatedData.playerId) + 1;
        ...
        const playersByLevel = allPlayers.reduce(
          (acc, player) => {
            if (!acc[player.level]) {
              acc[player.level] = [];
            }
