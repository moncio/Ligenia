...
import { IPerformanceHistoryRepository, PerformanceHistoryFilter } from '../../interfaces/repositories/performance-history.repository';
import { z } from 'zod';
...
export const getPlayerPerformanceHistorySchema = z.object({
  ...
  month: z.union([
    ...
    z.string().regex(/^([1-9]|1[0-2])$/).transform(val => parseInt(val, 10))
  ]).optional(),
  limit: z.union([
 ...
 */
export class GetPlayerPerformanceHistoryUseCase extends BaseUseCase<GetPlayerPerformanceHistoryInput, PerformanceHistory[]> {
  constructor(
   ...
   */
  protected async executeImpl(input: GetPlayerPerformanceHistoryInput): Promise<Result<PerformanceHistory[]>> {
    try {
      // Validate input
      ...
      const performanceHistory = await this.performanceHistoryRepository.findByUserId(validInput.userId, filter);
      ...
      return Result.fail<PerformanceHistory[]>(
        error instanceof Error 
          ? error 
          : new Error('Failed to get player performance history')
      );
    }
  }
} 